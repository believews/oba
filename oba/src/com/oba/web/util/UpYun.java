...
public class UpYun {
	 ...
	 * @throws IOException
	 */
	public  boolean writeFileV0(String filePath,File file) throws IOException {

		
		
		// 设置待上传文件的 Content-MD5 值
		// 如果又拍云服务端收到的文件MD5值与用户设置的不一致，将回报 406 NotAcceptable 错误
		setContentMD5(UpYun.md5(file));
		...
		boolean result = writeFile(filePath, file, true);
...
//		zmwhlj910
		...
		String bucket = "hlj-img";
		String userName = "zmwserver001";
		String pwd = "zmwhlj910";
		
		BufferedReader br = new BufferedReader(new FileReader("upyun.succ.txt"));
		String line = br.readLine();
		Set<String> succLines = new HashSet<String>();
		while(line!=null){
			succLines.add(line.trim());
			line = br.readLine();
		}
		br.close();
		System.out.println("old uploaded:"+succLines.size());
		PrintStream out = new PrintStream(new FileOutputStream("upyun.succ.txt",true));
				
		UpYun upyun =new UpYun(bucket,userName,pwd);
		
		//http://22.2.2.2.2/   zmw/upload/product/  xxx.lpp
		
		for(File f:dirF.listFiles()){
			
			if(succLines.contains(f.getName())){
				System.out.println(f.getName()+" uploaded,continue.");
				continue;
			}
			long t = System.currentTimeMillis();
			//boolean deleteSucc = upyun.deleteFile("/upload/product/"+f.getName());
			long t3 = System.currentTimeMillis();
			
			
			//System.out.println("delete:/upload/product/"+f.getName()+" "+deleteSucc+(t3-t)+" ms");
			
			boolean uploadSucc = upyun.writeFileV0("/zmw/upload/product/"+f.getName(), f);
		    if(uploadSucc){
		    	out.println(f.getName());
		    }
		    long t2 = System.currentTimeMillis();
		    
		    System.out.println("upload /zmw/upload/product/"+f.getName()+" "+uploadSucc+" "+(t3-t2)+" ms");
		}
		out.close();
